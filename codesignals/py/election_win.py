...
candidates so far, and an integer k equal to the number of voters 
who haven't cast their vote yet, find the number of candidates who 
still have a chance to win the election.

The winner of the election must secure strictly more votes 
than any other candidate. If two or more candidates receive the
...
def electionsWinners(votes, k):
    lead = max(votes)
    if k == 0 and votes.count(lead) == 1:
        return 1
    if votes.count(lead) > 1 and k == 0:
        return 0
    potentials = [x for x in votes if x+k > lead]
