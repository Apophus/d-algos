...
class User:
    ...
    def calc_progress(self, r):
        if r == 0 or r < -8 or r > 8 or self.rank < -8 or self.rank > 8 or self.rank == 0 or self.progress > 100\
                or self.progress < 0:
            raise ValueError
        if r == self.rank:
            return 3
        if r < 0:
            ...
            if self.rank < 0:
                ...
                if abs(self.rank - r + 1) >= 2 and self.rank > r:
                    ...
                    # add1 to r since 0 is not counted
                    return 10 * (self.rank - r + 1)**2
            else:
                if abs(self.rank - r) == 1 and self.rank > r:
                    return 1
                if abs(self.rank - r) >= 2 and self.rank > r:
